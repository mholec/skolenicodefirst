...
namespace Dotazovani.Priklady
{
    public class QuerySyntaxExamples
    {
        ...
        public void Start()
        {
            ...
            IQueryable<Book> books = from b in allBooks where b.BookId > 0 select b;
            Console.WriteLine(books.ToString());

            // postupné zpřesňování dotazu (tvorba sql query)
            books = from b in books where b.Title.Length > 10 select b;
            Console.WriteLine(books.ToString());
            ...
            List<Book> good = (from b in context.Books where b.Title.Contains("a") && b.Added > DateTime.Now select b).ToList();
            List<Book> bad = (from b in
                                (from b in context.Books where b.Title.Contains("a") select b).ToList()
                                where b.Added > DateTime.Now
                                select b).ToList();
